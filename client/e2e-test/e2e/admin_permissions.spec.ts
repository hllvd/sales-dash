...
test.describe('Admin Scoped Permissions (TEAR 3)', () => {
  ...
  test('1. Setup: SuperAdmin creates Admin, Child, and other test users', async ({ page }) => {
    ...
    await page.waitForTimeout(500);
    const regPromiseNoteam = page.waitForResponse(r => r.url().includes('/api/users/register'), { timeout: 15000 }).catch(() => null);
    await page.click('button:has-text("Criar Usuário")');
    ...
    await page.waitForTimeout(500);
    const regPromiseIneligible = page.waitForResponse(r => r.url().includes('/api/users/register'), { timeout: 15000 }).catch(() => null);
    await page.click('button:has-text("Criar Usuário")');
